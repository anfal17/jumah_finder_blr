...
import AddMasjidModal from '../components/modals/AddMasjidModal';
import { getUserLocation, calculateDistance, formatDistance } from '../utils/location';

// Nearby Mosques Dropdown
const NearbyDropdown = ({ masjids, userLocation, onSelect, onClose }) => {
...
const SearchDropdown = ({ masjids, searchQuery, onSelect, userLocation }) => {
    const filteredMasjids = useMemo(() => {
        if (!searchQuery || searchQuery.trim() === '') return [];
        const query = searchQuery.toLowerCase();
        let results = masjids.filter(m =>
        ...
        return results;
    }, [searchQuery, userLocation, masjids]);
    ...
    return (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden z-50 max-h-80 overflow-y-auto">
            {filteredMasjids.map((masjid) => (
                <button
                    key={masjid._id || masjid.id} // Handle Mongo _id or fallback
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-800 text-sm truncate">{masjid.name}</p>
                        <p className="text-xs text-slate-500">
                            {masjid.area ? `${masjid.area}, Bengaluru` : (masjid.distance ? formatDistance(masjid.distance) + ' away' : 'Bengaluru')}
                        </p>
                    </div>
...
// Report Modal moved to separate file
        ...
        <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4" onClick={onClose}>
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full" onClick={(e) => e.stopPropagation()} style={{ animation: 'modalSlideUp 0.3s ease-out' }}>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-emerald-100 text-sm mt-1">
                                {masjid.area ? `${masjid.area}, Bengaluru` : 'Bengaluru'}
                ...
                <div className="px-6 pb-6 space-y-3">
                    <a
                        href={masjid.mapLink || `https://www.google.com/maps/dir/?api=1&destination=${masjid.lat},${masjid.lng}`}
                        target="_blank"
...
const HamburgerMenu = ({ isOpen, onClose, onAddMasjid, onFeedback }) => {
...
function Home() {
    ...
    useEffect(() => {
        const fetchMasjids = async () => {
            try {
                ...
                // Fallback to empty or show error? For now, empty is fine or retry logic
            }
        };
        fetchMasjids();
    }, []);
    ...
    return (
        <div className="relative w-screen h-screen overflow-hidden bg-slate-50">
            {showAddMasjidModal && <AddMasjidModal onClose={() => setShowAddMasjidModal(false)} />}
                    ...
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="relative">
                            ...
                            {showDropdown && searchQuery && (
                                <SearchDropdown masjids={masjids} searchQuery={searchQuery} onSelect={handleSelectMasjid} userLocation={userLocation} />
                            )}
                        </div>
                    </div>
                </div>
            ...
            <HamburgerMenu
                ...
                onAddMasjid={() => setShowAddMasjidModal(true)}
